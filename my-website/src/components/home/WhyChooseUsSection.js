          ...
          {whyChooseUsData.map((item, index) => {
            ...
            return (
              <ScrollReveal key={item.id} delay={index * 0.06} variant="fade-up" className="h-full">
                <div className="group relative rounded-3xl bg-white dark:bg-[#0A0F0C] border border-slate-200 dark:border-[#1E2E25] p-6 sm:p-8 transition-all duration-300 hover:border-[#10B981]/60 hover:shadow-[0_10px_30px_rgba(16,185,129,0.15)] hover:-translate-y-1 flex flex-col justify-between h-full overflow-hidden shadow-sm dark:shadow-none">
                  {/* Subtle Background Glow Effect on Hover */}
                  <div className="absolute -right-12 -top-12 w-32 h-32 bg-[#10B981]/5 rounded-full blur-2xl group-hover:bg-[#10B981]/15 transition-all duration-500 pointer-events-none" />

                  <div>
                    {/* Header Row inside Card: Icon & Optional Badge */}
                    <div className="flex items-center justify-between mb-6">
                      <div className="w-12 h-12 rounded-2xl bg-slate-100 dark:bg-[#111C16] border border-slate-200 dark:border-[#10B981]/30 text-[#10B981] flex items-center justify-center group-hover:scale-110 group-hover:bg-[#10B981] group-hover:text-white transition-all duration-300 shadow-sm">
                        <IconComponent size={24} />
                      </div>

                      {item.badge && (
                        <span className="inline-flex items-center gap-1.5 text-[11px] font-mono font-semibold text-[#10B981] bg-[#10B981]/10 px-2.5 py-1 rounded-full border border-[#10B981]/30 shadow-[0_0_10px_rgba(16,185,129,0.15)]">
