        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 relative">
          {processSteps.map((step, idx) => (
            <AnimatedSection
              ...
              direction="up"
              delay={idx * 0.08}
              className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl p-7 relative flex flex-col justify-between group hover:border-[#6366F1]/40 transition-all duration-300 hover:-translate-y-1"
            >
              <div>
                <div className="flex items-center justify-between mb-6">
                  <span className="text-3xl font-black font-mono bg-gradient-to-r from-[#6366F1] to-[#a855f7] text-transparent bg-clip-text">
                    {step.step}
                  </span>
                  <div className="w-8 h-8 rounded-full bg-[#6366F1]/10 border border-[#6366F1]/20 text-[#6366F1] text-xs font-bold flex items-center justify-center">
                    STEP
                  </div>
                </div>

                <h3 className="text-xl font-bold text-[var(--text-heading)] mb-3 group-hover:text-[#6366F1] transition-colors">
                  {step.title}
                </h3>
