          ...
          {faqs.map((faq, idx) => {
            ...
            return (
              <AnimatedSection
                ...
                direction="up"
                delay={idx * 0.05}
                className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-2xl overflow-hidden transition-all duration-300 hover:border-[#6366F1]/40"
              >
                <h3>
                  <button
                    id={buttonId}
                    ...
                    className="w-full text-left p-6 flex items-center justify-between gap-4 font-bold text-base sm:text-lg text-[var(--text-heading)] cursor-pointer focus:outline-none focus:ring-2 focus:ring-[#6366F1]/50 rounded-2xl"
                  >
                    <span className="flex items-center gap-3">
                      <HelpCircle size={20} className="text-[#6366F1] shrink-0" />
