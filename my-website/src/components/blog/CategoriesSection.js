...
/**
 * CategoriesSection — Server-rendered category directory.
...
export default function CategoriesSection({ categoryCounts = {}, headingLevel = "h2" }) {
  ...
  return (
    <section className={`${isH1 ? "pt-24 pb-12 md:pt-28 md:pb-16" : "py-12 md:py-16"} px-4 sm:px-6 lg:px-8 bg-[var(--background)]`} aria-labelledby="categories-heading">
      <div
      ...
      >
            ...
            return (
              <div key={index}>
                <Link
                  ...
                  className={`group block p-6 rounded-2xl border border-[var(--card-border)] bg-gradient-to-br ${category.gradient} card-interactive no-underline h-full`}
                >
                  {/* Icon + Badge Row */}
                  <div className="flex items-center justify-between w-full mb-4">
                    <div className={`flex items-center justify-center w-12 h-12 rounded-xl bg-[var(--card-bg)] border border-[var(--card-border)] ${category.accentColor}  transition-colors duration-200 shrink-0`}>
                      <span className="inline-flex transition-transform duration-200 motion-safe:group-hover:scale-110">
                        {category.icon}
                      </span>
                    </div>
        ...
        <div className="text-center">
          <Link
            href="/categories"
            className="group inline-flex items-center gap-2 px-8 py-4 rounded-full bg-[var(--card-bg)] border border-[var(--card-border)] text-[var(--foreground)] font-bold transition-colors duration-200 hover:border-[#F59E0B]/40 hover:text-[#F59E0B] no-underline"
          >
            Explore All Categories
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="transition-transform duration-200 motion-safe:group-hover:translate-x-1"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
          </Link>
        </div>
      </div>
    </section>
